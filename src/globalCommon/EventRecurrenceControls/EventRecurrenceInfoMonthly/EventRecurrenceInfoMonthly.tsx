...
export class EventRecurrenceInfoMonthly extends React.Component<IEventRecurrenceInfoMonthlyProps, IEventRecurrenceInfoMonthlyState> {
  ...
  public constructor(props: IEventRecurrenceInfoMonthlyProps) {
    ...
    this.state = {
      ...
      endDate:   this.props?.DueDate ?? moment(this.props.startDate).add('month', 1).toDate(),
      // endDate:undefined,
      numberOcurrences: '10',
  ...
  private async applyRecurrence() {
   ...
   // console.log(recurrenceXML);
    this.props.returnRecurrenceData(this.state.startDate, this.state.endDate, recurrenceXML,this.state?.selectedPeople,this.state?.selectedPeopleId,this.state?.selectedPeopleEmail,this.state.PatternType,this.state.selectdateRangeOption,this.props?.UserDataIndex);
  }
  /**
   *
   * @private
